...
class PageGetter(object):
    ...
    def getPage(self,
        ...
        **Keyword arguments:**
        ...
        logger.info("%s:%s:checking negative cache" % (url, time.time() - start))
        ...
        logger.info("%s:%s:getting page from RQ" % (url, time.time() - start))
        if request_kwargs["method"] != "GET":
            ...
            data = yield self._getPage(
                    ...
                    confirm_cache_write, 
                    host)
        logger.info("%s:%s:Checking for stale content" % (url, time.time() - start))
        ...
        logger.info("%s:%s:Returning" % (url, time.time() - start))
        returnValue(data)
    ...
    def _getCachedDataCallback(self, cached_data):
        response = json.loads(decompress(cached_data[1][1]))
        if len(response) == 0:
            raise Exception("Empty cached data.")
        data = {
            "headers": json.loads(decompress(cached_data[0][1])),
            "response": response,
        }
        return data
    ...
    def _requestWithNoCacheHeadersErrback(self,
        ...
        deferreds.append(self.redis_client.delete(http_key, compress(json.dumps(""), 1)))
        d = DeferredList(deferreds, consumeErrors=True)
    ...
    def _handleRequestWithCacheHeadersError(self,
        ...
        if status == 304:
            ...
            headers["request-failures"] = ",".join(http_history["request-failures"])
            headers_key = 'headers:%s' % request_hash
            d = self.redis_client.set(headers_key, compress(json.dumps(headers), 1))
            if confirm_cache_write:
